SET TIME ZONE 'Europe/Amsterdam';


with bucket_minmax as (with buckets as (SELECT generate_series(
                                                       date_trunc('day', now()) - interval '13 day',
                                                       date_trunc('day', now()),
                                                       interval '1 day') as day)
                       select b.day,
                              min(d.tm) as min_ts,
                              max(d.tm) as max_ts
                       from buckets b
                                left join data d on d.tm >= b.day and d.tm < b.day + interval '1 day'
                       group by 1)
select bmm.day::timestamp as time,
       coalesce(dmax.p1 - dmin.p1, 0) as p1,
       coalesce(dmax.p2 - dmin.p2, 0) as p2,
       coalesce(dmin.n1 - dmax.n1, 0) as n1,
       coalesce(dmin.n2 - dmax.n2, 0) as n2,
       coalesce(dmax.p1 - dmin.p1, 0) + coalesce(dmax.p2 - dmin.p2, 0) as pos,
       coalesce(dmin.n1 - dmax.n1, 0) + coalesce(dmin.n2 - dmax.n2, 0) as neg
from
    bucket_minmax bmm
        left join data dmin on bmm.min_ts = dmin.tm
        left join data dmax on bmm.max_ts = dmax.tm
order by
    day asc

------

select * from data
order by tm desc

SET TIME ZONE 'Europe/Amsterdam';

with buckets as (
    SELECT generate_series(
                   date_trunc('year', now())::date,
                   (date_trunc('year', now())::date + interval '11 month')::date,
                   interval '1 month'
           )::timestamp as day )
select
    b.day,
    date_trunc('day', b.day)
from
    buckets b